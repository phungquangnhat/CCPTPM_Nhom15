...
<body>
	...
	<div id="themDon">
			<div >
				<h2 id="h2ThemDon">Thêm Đơn Đặt Hàng</h2>
			</div>
			<div id="themDonCon">
				<div id="divGroup">
        			...
        			$database = "ql_fastfood";
       				$conn = mysqli_connect($servername, $username, $password, $database);
        				if(!$conn)
        				{
        					...
        					$query = "select count(*) as tong from dathang";
				...
				<div id="divGroup">
				  ...
				  <input type="text" class="form-control" name="txtSDT" id="txtSDT" placeholder="Nhập số điện thoại" onfocusout="kiemTraSDT()">
				</div>
				...
				<div id="divGroup">
				  ...
				  <input type="text" class="form-control" name="txtDiaChi" id="txtDiaChi" placeholder="Địa chỉ giao hàng (Không ship ngoại tỉnh)">
				</div>
				<div id="divGroup">
				  <label>Ghi chú:</label>
				  <input type="text" class="form-control" name="txtGhiChu" id="txtGhiChu" placeholder="Nhập thông tin">
				</div>
				<div id="divGroup">
				...
				<button type="submit" class="btn btn-success" id="btthem" name="btthem"> <i class="fas fa-plus-circle"></i> Thêm</button>
				<button type="submit" id="btThem" name="btThem">Chọn</button>
				<a href="index.php"> <i class="fas fa-undo"></i> Quay lại</a>	
	...
	<?php
		if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['btThem']))
		{
			...
			$query = "insert into dathang(hoTen, sdt, diaChi, note, ngayDat, trangThai) 
			values('".$hoTen."','".$sdt."','".$diaChi."','".$ghiChu."','".$ngayDat."', '".$trangThai."')";
			echo $query;
			$result = mysqli_query($conn, $query);
			if ($result == true) {
	...
	<div id="chiTietDon">
			...
			<div id="chiTietCon">
				...
				<div id="divGroup">
				  <label>Mã đơn đặt hàng:<i id="iGroup">*</i></label>
				  <?php
				  	echo '<input  type="text" class="form-control" name="txtMdh" id="txtMdh" value = "'.($tong - 1).'">';
				  ?>
				</div>
				<div id="divGroup">
	...
	<div id="tblChiTiet">
    	<?php
	        ...
	        if(!$conn)
	        {
	            ...
	            $query = "select * from chitietdathang where DatHang_id = '".($tong - 1)."' order by DatHang_id";
	            $result = mysqli_query($conn, $query);      
	            if(mysqli_num_rows($result) > 0)
	            {
	                //tạo bảng
